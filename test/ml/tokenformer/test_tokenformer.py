...
def test_tokenformer_model_equivalence(model_setup, input_text):
    ...
    is_close, max_diff = compare_model_outputs(model1, model2, tokenizer, input_text, device)
    
    assert is_close, f"Outputs are not close for input: {input_text}"
    assert max_diff < 1e-3, f"Maximum difference {max_diff} is too large for input: {input_text}"