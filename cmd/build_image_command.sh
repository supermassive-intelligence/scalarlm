...
docker_build_command="docker build --platform ${docker_platform} --build-arg BASE_NAME=${target} --build-arg TORCH_CUDA_ARCH_LIST=${sm_arch} --build-arg VLLM_TARGET_DEVICE=${vllm_target_device} --build-arg VLLM_SOURCE=local -t cray:latest ."
