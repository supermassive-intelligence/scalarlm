...
async def delete(job_hash: str):
    logger.info(f"Delete request received for job hash: {job_hash}")
    ...
    try:
        ...
        logger.info(f"Job status before deletion: {job_status}")

        try:
            # Cancel SLURM job first if it's still running
        ...
        logger.exception(
            f"Error deleting training job {job_hash} "
