#!/usr/bin/env bash
# Repeated runs at N=100 to get statistics for scalarlm /v1/generate vs
# openai /v1/completions array. First iteration of each scenario is a
# warmup and excluded; next ITER runs are measurement.
#
# Usage:
#   bench/scenarios/n100_repeat.sh <model> <platform-label> [base-url]
...
ITER="${N100_ITERATIONS:-10}"
WARMUP="${N100_WARMUP:-1}"

if [[ -z "${MODEL}" || -z "${PLATFORM}" ]]; then
  echo "usage: $0 <model> <platform-label> [base-url]" >&2
  exit 2
fi

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
OUT="${ROOT}/bench/results/${PLATFORM}-n100-repeat/${STAMP}"
