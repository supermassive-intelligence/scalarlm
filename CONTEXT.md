...
Adapters are hot-loaded into a running engine by name (no restart) via vLLM's
`/v1/load_lora_adapter`, and are discovered as `.pt` files in the training job directory.
...
_Avoid_: calling it a LoRA (it's a distinct mechanism, served *as* a LoRA at inference).
...
calling, chat templates (c), or Tokenformer training compatibility (d). Requires the
full cray stack.
