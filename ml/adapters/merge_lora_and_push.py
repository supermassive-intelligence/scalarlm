...
def _backfill_missing_weights(output_dir: Path, base_model_name: str) -> None:
    """
    ...
    This corrects for truncated-architecture bugs where the local transformers
    version materialises fewer layers than the canonical checkpoint (e.g.
    AutoModelForCausalLM loading an older Gemma4 variant with 15 instead of 35
    language layers).  The HF base weights are used verbatim for those layers —
    LoRA deltas were not applied to them — which is still far better than an
    incomplete file that fails to load at all.
    """
