...
def replace_layers(model, custom_layer_class, logger=logging.getLogger(__name__)):
    """In-place layer modification - avoid expensive layer creation"""
...
# Optimized count_parameters function
def count_parameters(module):
    """Count parameters more efficiently"""
    start_time = time.time()
    count = sum(p.numel() for p in module.parameters())
    elapsed = time.time() - start_time
    logging.getLogger(__name__).debug(f"Parameter counting took {elapsed:.3f}s")
    return count